...
class Finetune:
    def __init__(self, config):
        ...
        self.char_to_index, self.index_to_char, self.vocab_size = get_char_maps(
            properties.char_set)
        self.crnn = CRNN(self.vocab_size, False).to(self.device)
        self.crnn.load_state_dict(torch.load(config.crnn_model_path))
        ...
        from Deblurring.data.docdata_finetuning import DocDataFinetuning
        from Deblurring.data.docdata import DocData

        dataset_train = DocDataFinetuning(self.path_train_img, 
                                          self.path_train_gt, 
                                          config.IMAGE_SIZE, 
                                          config.text_dir_train,
                                          True)
        self.batch_size = config.BATCH_SIZE
        self.dataloader_train = DataLoader(dataset_train, batch_size=self.batch_size, 
                                           shuffle=True, drop_last=False,
                                           num_workers=config.NUM_WORKERS,
                                           collate_fn=DocDataFinetuning.collate)
        
        dataset_test = DocData(config.TEST_PATH_IMG, 
                                         config.TEST_PATH_GT, 
                                         config.IMAGE_SIZE,0)
        ...
        if self.mode >= 1  and self.continue_training == 'True':
            ...
            self.bestLPIPS = checkpoint_denoiser['bestLPIPS']
            self.bestDISTS = checkpoint_denoiser['bestDISTS']
        ...
        self.computeSSIM    =  False
        self.computeLPIPS   =  False
        self.computeDISTS   =  False
        self.computeMANIQA  =  False
        self.computeMUSIQ   =  False
        ...
        with torch.no_grad():
            ...
            for img, gt, name in tq:
                ...
                if self.computeMANIQA:
                    maniqa = self.maniqa(finalImgs.to(self.device)).item()
                    test_results["maniqa"].append(maniqa)
                if self.computeMUSIQ:
                    musiq = self.musiq(finalImgs.to(self.device)).item()
                    test_results["musiq"].append(musiq)
                if self.computeMANIQAGT:
                    maniqa_gt = self.maniqa(gt.to(self.device)).item()
                    test_results["maniqa_gt"].append(maniqa_gt)
                if self.computeMUSIQGT:
                    musiq_gt = self.musiq(gt.to(self.device)).item()
                    test_results["musiq_gt"].append(musiq_gt)


                if iteration==self.val_iterations:
                    break
            ...
            if ave_psnr > self.bestPSNR:
                ...
                to_save = {
                        ...
                        'bestLPIPS': self.bestLPIPS if self.bestLPIPS > ave_lpips else ave_lpips,
                        'bestDISTS': self.bestDISTS if self.bestDISTS > ave_dists else ave_dists,
                }
            ...
            if ave_lpips < self.bestLPIPS:
                self.bestLPIPS = ave_lpips
                to_save = {
                        'iteration': current_iteration,
                        'model_state_dict': self.network.init_predictor.state_dict(),
                        'optimizer_state_dict': self.optimizer.state_dict(),
                        'bestPSNR': self.bestPSNR if self.bestPSNR > ave_psnr else ave_psnr,
                        'bestLPIPS': self.bestLPIPS,
                        'bestDISTS': self.bestDISTS if self.bestDISTS > ave_dists else ave_dists,
                }
                torch.save(to_save, #self.network.init_predictor.state_dict(),
                    os.path.join(self.weight_save_path, f'BEST_LPIPS_model_init.pth'))
                to_save = {
                        'iteration': current_iteration,
                        'model_state_dict': self.network.denoiser.state_dict(),
                        'optimizer_state_dict': self.optimizer.state_dict(),
                ...
                torch.save(to_save,#self.network.denoiser.state_dict(),
                    os.path.join(self.weight_save_path, f'BEST_LPIPS_model_denoiser.pth'))

            if ave_dists < self.bestDISTS:
                self.bestDISTS = ave_dists
                to_save = {
                        'iteration': current_iteration,
                        'model_state_dict': self.network.init_predictor.state_dict(),
                        'optimizer_state_dict': self.optimizer.state_dict(),
                        'bestPSNR': self.bestPSNR if self.bestPSNR > ave_psnr else ave_psnr,
                        'bestLPIPS': self.bestLPIPS if self.bestLPIPS > ave_lpips else ave_lpips,
                        'bestDISTS': self.bestDISTS,
                }
                torch.save(to_save,# self.network.init_predictor.state_dict(),
                    os.path.join(self.weight_save_path, f'BEST_DISTS_model_init.pth'))
                to_save = {
                        'iteration': current_iteration,
                        'model_state_dict': self.network.denoiser.state_dict(),
                        'optimizer_state_dict': self.optimizer.state_dict(),
                ...
                torch.save(to_save,#self.network.denoiser.state_dict(),
                    os.path.join(self.weight_save_path, f'BEST_DISTS_model_denoiser.pth'))
    
    # CALL CRNN
    def _call_crnn_model(self, images, labels):
        X_var = images.to(self.device)
        scores = self.crnn(X_var)
    ...
    def finetune(self):
        ...
        while iteration < self.iteration_max:
            ...
            for img, gt, labels, name in tq:
                ...
                final_imgs = (noise_pred + init_predict)
                
                #EXTRACT IMG PATCHES CONTAINING TEXT. THE EXTRACTION IS CONDITIONED
                #ON THE BOUNDING BOX CONSTRUCTED AT PREPROCESSING TIME
                text_crops, labels_stacks = extract_patches_with_labels(
                    final_imgs, labels, self.input_size
                )
                
                #IF IMAGE DOES NOT CONTAIN TEXT THEN SKIP
                if text_crops == []:
                    ctc_loss = torch.tensor(0)
                    
                #OTHERWISE CALL CRNN MODEL
                else:
                    scores, y, pred_size, y_size = self._call_crnn_model(
